(async function() {
  ...
  query {
    user(login: "${USERNAME}") {
      ...
      repositories(first: 20, orderBy: {field: PUSHED_AT, direction:DESC}, affiliations: [OWNER]) {
        totalCount
        nodes {
          name
          url
...
<svg aria-label="star" role="img" viewBox="0 0 16 16" height="16" width="16" class="">
  ...
  ></path>
    ...
    const updatedAt = `Updated on ${date.getDate()} ${months[date.getMonth()]}${thisYear ? "" : ", " + date.getFullYear() }`
